...
src/
  ...
  features/  one user action per slice, verb-named (join-call, like-post)
  entities/  domain data + CRUD (call, user) — no interaction logic
...
if it's violated. See `src/entities/call/`, `src/features/join-call/`,
`src/widgets/call-room/`, `src/pages/call/` for a working example of the
...
folders:**
```bash
bun run new:slice -- --layer=features --name=leave-call
```
